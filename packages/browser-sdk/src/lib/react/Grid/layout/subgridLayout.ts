...
import { type Bounds, type Box, type CellProps, type SubgridLayout } from "./types";

// Calculate how much we need to move the incomplete row/col so it becomes centered:
interface GetCenterPaddingOptions {
    index: number;
    isPortrait: boolean;
    rows: number;
    cols: number;
...
function getCenterPadding({ index, isPortrait, rows, cols, cellBounds, cellCount }: GetCenterPaddingOptions) {
...
export function calculateLayout({
    ...
    cellBounds,
    paddings = makeBox(),
}: CalculateLayoutOptions): SubgridLayout {
    let width = 0;
    let height = 0;
    let rows = 0;
    let cols = 0;
...
export function getCellPropsAtIndexForLayout({ index, layout }: { index: number; layout: SubgridLayout }): CellProps {
