...
import { EnhancedEventEmitter } from "mediasoup-client/lib/enhancedEvents";
...
import type { IceParameters, DtlsRole } from "mediasoup-client/lib/Transport";
import type { RtpCapabilities, MediaKind, ExtendedRtpCapabilities } from "mediasoup-client/lib/RtpParameters";
import type { SctpCapabilities, SctpStreamParameters } from "mediasoup-client/lib/SctpParameters";
import type {
    HandlerFactory,
    HandlerInterface,
    HandlerEvents,
	...
	implements HandlerInterface
{
	...
	static createFactory(): HandlerFactory {
		return {
			name: NAME,
			factory: (options: HandlerOptions): Safari17 => new Safari17(options),
			getNativeRtpCapabilities: async (): Promise<RtpCapabilities> => {
				...
				try {
					...
					const nativeRtpCapabilities =
						Safari17.getLocalRtpCapabilities(sdpObject);
	...
	async send({
		...
		const nativeRtpCapabilities =
			Safari17.getLocalRtpCapabilities(localSdpObject);
