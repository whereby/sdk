 ...
 *
 * <sampleDirectory> - root directory, sensible named, eg using the date of capture
...
async function main(sampleDirectory) {
    ...
    Object.entries(SUMMARY).forEach(([reportType, properties]) => {
        const maxLength = ["Property", ...Object.keys(properties)].reduce((maxLength, currentString) => {
            return Math.max(maxLength, currentString.length);
        }, 0);

        const header = `| ${"Property".padEnd(maxLength)} | ${browsers.join(" | ")} |`;
        const separator = `| ${"".padEnd(header.length - 4, "-")} |`;
        const rows = [];
        ...
        Object.entries(properties).forEach(([statName, browserSupport]) => {
            ...
            browsers.forEach((b) => {
                row.push(browserSupport[b] ? "✅".padEnd(b.length) : "❌".padEnd(b.length));
            });
